...
const EmailVerificationFR = ({route}) => {
  ...
  const postTokenToServer = async (token, username) => {
    ...
    const response = await fetch(`http://${API_IP_ADDRESS}/saveExpoPushTokenFR`, {
          method: 'POST',
          headers: {
...
const registerResponse = await fetch(`http://${API_IP_ADDRESS}/registerRestaurant`, {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
...
await sendTokenToServerAfterRegistration(username);
      ...
      navigation.navigate('RestaurantLogin', { email: email });
    } else {
...
export default EmailVerificationFR;
